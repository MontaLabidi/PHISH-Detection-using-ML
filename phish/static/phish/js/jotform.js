            ...
            if ((condition.link.toLowerCase() == 'any' && any) || (condition.link.toLowerCase() == 'all' && all)) {
                ...
                if (window.FORM_MODE == 'cardform') {
                  if (action.skipTo == 'end') {
                    ...
                    var nextQuestionIndex = allQuestions.findIndex(function(q) { return q.id === action.skipTo;});
                    if(nextQuestionIndex && allQuestions[nextQuestionIndex].type === 'control_head') {
                      nextQuestionIndex++;
                      JotForm.conditions.forEach(function(cond) {
                        if(cond.action[0].field === allQuestions[nextQuestionIndex].id) {
                          nextQuestionIndex++;
                        }
                      });
        ...
        switch (resultFieldType) {
            case "html":
                try {
                    ...
                    var className = result + "_" + calc.replaceText.replace(/\[/g,"_field_");
